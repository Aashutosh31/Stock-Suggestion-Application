import React from 'react'; // Removed useState, as it's now internal
import { NavLink, useNavigate } from 'react-router-dom';
import { useTheme } from '../context/ThemeContext';
import toast from 'react-hot-toast';

// --- 1. ADD A "CLOSE" ICON FOR THE MOBILE MENU ---
const Icons = {
    ...
    Close: (props) => <svg {...props} xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>,
};
...
// --- 2. ACCEPT MOBILE PROPS ---
const Sidebar = ({ isCollapsed, toggleCollapse, isMobileOpen, setIsMobileOpen }) => { 
    ...
    // --- 3. DYNAMIC CLASSES FOR RESPONSIVENESS ---
    const sidebarBg = 'dark:bg-gray-800 bg-white border-r dark:border-gray-700 border-gray-200';
    
    // Desktop width (for the <main> margin)
    const sidebarWidth = isCollapsed ? 'w-20' : 'w-64'; 
    
    // Mobile width (always full-width when open)
    const mobileSidebarWidth = 'w-64'; // Or 'w-full', 'w-3/4' etc.
    ...
    // On desktop, links center when collapsed. On mobile, they are never collapsed.
    const linkTextClass = (isMobile) => 
        `transition-opacity duration-300 ${isCollapsed && !isMobile ? 'opacity-0 absolute left-full whitespace-nowrap' : 'opacity-100 ml-3'}`;

    const linkClasses = ({ isActive }) =>
        `flex items-center justify-start relative p-3 rounded-lg transition-all duration-300 group ${
            isCollapsed ? 'md:justify-center' : '' // Only justify-center on desktop
        } ${
            isActive 
    ...
    // --- 5. SHARED SIDEBAR CONTENT (used by both mobile and desktop) ---
    const SidebarContent = ({ isMobile = false }) => (
        <div className={`flex flex-col h-full ${sidebarBg} shadow-xl p-4`}>
            {/* Logo and Toggle Button */}
            <div className={`flex items-center ${isCollapsed && !isMobile ? 'justify-center' : 'justify-between'} h-16 mb-6`}>
                
                {/* Logo: Hide if collapsed on desktop */}
                ...
                <button
                    ...
                    className={`p-2 rounded-full dark:hover:bg-gray-700 hover:bg-gray-100 dark:text-white text-gray-800 ${isMobile ? 'hidden' : 'hidden md:block'}`}
                    title={isCollapsed ? "Expand Sidebar" : "Collapse Sidebar"}
                >
                    <Icons.Menu className="w-6 h-6" />
                </button>

                {/* Mobile Close Button: Hide on desktop */}
                <button
                    onClick={() => setIsMobileOpen(false)}
                    className="p-2 rounded-full dark:hover:bg-gray-700 hover:bg-gray-100 dark:text-white text-gray-800 md:hidden"
                    title="Close Menu"
                >
            ...
            {/* Navigation Links */}
            <nav className="flex-grow space-y-2" onClick={() => isMobile && setIsMobileOpen(false)}>
                {navItems.map((item) => (
                    <NavLink
                        key={item.name}
                        to={item.to}
                    ...
                    >
                        ...
                        <span className={linkTextClass(isMobile)}>
                            {item.name}
                        </span>
                    </NavLink>
                ))}
            ...
            <div className="mt-auto space-y-4">
                <button
                    onClick={toggleTheme}
                    className={`w-full flex items-center ${isCollapsed && !isMobile ? 'justify-center' : 'justify-start'} ${themeButtonClasses} ${
                        theme === 'dark' 
                ...
                <button
                    ...
                    className={`w-full flex items-center ${isCollapsed && !isMobile ? 'justify-center' : 'justify-start'} p-3 rounded-lg bg-red-600 text-white font-semibold hover:bg-red-700 transition duration-200 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 dark:focus:ring-offset-gray-900`}
                    title="Log Out"
                >
    ...
    return (
        <>
            ...
            {/* This is the overlay that slides in on mobile */}
            <div 
                className={`fixed top-0 left-0 z-40 h-full ${mobileSidebarWidth} transition-transform duration-300 ease-in-out md:hidden ${
            ...
            {/* --- 7. DESKTOP SIDEBAR (Static) --- */}
            {/* This is the permanent sidebar for medium screens and up */}
            <div 
                className={`fixed top-0 left-0 h-full transition-all duration-300 z-30 hidden md:flex ${sidebarWidth}`}
            >
